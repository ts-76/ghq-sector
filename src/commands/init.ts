...
export async function runInit(options: InitOptions) {
  const ghqRoot = expandHome(
    options.ghqRoot ??
      (options.yes
        ? await getGhqRoot()
        : await prompt("ghq root", await getGhqRoot())),
  );
  ...
  const format =
    ...
    (options.yes
      ? "json"
      : (await prompt("config format (json/yaml)", "json")) === "yaml"
        ? "yaml"
        : "json");
  const owner = options.yes
    ? await getDefaultOwnerForYesMode()
    : await selectDefaultOwner();

  const config = normalizePortableConfig(
    createConfigTemplate(ghqRoot, workspaceRoot, owner ?? undefined),
  );
  ...
  const codeWorkspacePath = await generateCodeWorkspace({
    ...config,
    workspaceRoot,
  });
