...
import { hasGh, listGhOwnerCandidates, listGhRepositories } from '../shared/gh.js';
...
function resolveUiDistRoot() {
  ...
  return [path.join(compiledRoot, 'ui-dist'), path.join(packageRoot, 'dist', 'ui-dist'), path.join(uiRoot, 'dist')];
}
...
export async function runEdit(options: EditOptions) {
  const loaded = await loadConfig(options.config ? path.resolve(process.cwd(), options.config) : process.cwd());
  const uiDistRoot = await ensureUiBuild();

  const host = options.host ?? '127.0.0.1';
  const port = options.port ?? 4173;
  const server = await createEditServer({ host, port, uiDistRoot, configPath: loaded.path });
...
async function handleRequest(
  ...
  if (method === 'GET' && url.pathname === '/api/repo-template') {
    const current = await loadConfig(configDir);
    sendJson(response, 200, { ok: true, result: createRepoTemplate(current.config) });
    return;
  }
  ...
  if (method === 'GET' && url.pathname === '/api/gh/repos') {
    ...
    if (!ghAvailable) {
      sendJson(response, 200, { ok: true, available: false, accounts: [], repositories: [] });
      return;
    }
    ...
    if (!owner) {
      sendJson(response, 200, { ok: true, available: true, accounts, repositories: [] });
      return;
    }

    const repositories = await listGhRepositories(owner);
    sendJson(response, 200, { ok: true, available: true, owner, accounts, repositories });
    return;
  }

  if (method === 'POST' && url.pathname === '/api/repos') {
    const payload = (await readJsonBody(request)) as { repo?: Record<string, unknown> };
    const current = await loadConfig(configDir);
    const currentValue = current.config as unknown as Record<string, unknown>;
    const nextRepos = Array.isArray(currentValue.repos) ? [...currentValue.repos] : [];
...
async function serveUiAsset(response: ServerResponse, uiDistRoot: string, pathname: string) {
  const normalizedPath = pathname === '/' ? '/index.html' : pathname;
  const filePath = path.join(uiDistRoot, normalizedPath.replace(/^\//, ''));
  const assetPath = (await fileExists(filePath)) ? filePath : path.join(uiDistRoot, 'index.html');
  const content = await readFile(assetPath);
  response.statusCode = 200;
  response.setHeader('content-type', mimeTypes.get(path.extname(assetPath)) ?? 'application/octet-stream');
...
async function ensureUiBuild() {
  ...
  throw new Error('ui build not found. run `bun run build` in ./ui before using `gsec edit`, or install Bun so the UI can be built automatically.');
}
