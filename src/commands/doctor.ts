...
export async function runDoctor(cwd = process.cwd()): Promise<DoctorResult> {
  ...
  const push = (level: DoctorCheck['level'], scope: string, message: string) => {
  ...
  push('success', 'gh', `ok (${ownerCandidates.length} github.com account${ownerCandidates.length === 1 ? '' : 's'})`);
  if (ownerCandidates.length > 0) {
    push(
      'info',
      'gh accounts',
      ownerCandidates.map((candidate) => `${candidate.login}${candidate.active ? ' (active)' : ''}`).join(', '),
    );
  }

  const activeCandidate = ownerCandidates.find((candidate) => candidate.active)?.login ?? null;
  ...
  if (expandedConfigGhqRoot !== detectedGhqRoot) {
    push('warn', 'config ghqRoot', `differs from detected ghq root (${config.ghqRoot})`);
  }
  ...
  for (const repo of config.repos) {
    ...
    if (!config.categories.includes(repo.category)) {
      push('warn', `repo ${label}`, `category ${repo.category} is not declared in categories`);
    }
  ...
  if (!workspaceRootExists) {
    push('info', 'workspace checks', 'skipped because workspace root does not exist yet');
  } else {
    for (const resource of config.resources ?? []) {
    ...
    for (const repo of config.repos) {
      ...
      try {
        ...
        if (!stat.isSymbolicLink()) {
          push('warn', `repo ${label}`, `destination is not a symlink (${dest})`);
          continue;
        }
...
function checkDefaults(
  ...
  if (config.defaults?.owner) {
    const ownerLabel = config.defaults.owner === activeCandidate ? ' (active)' : '';
  ...
  if (candidateCount > 0) {
    push('warn', 'shorthand clone', 'interactive owner selection available, or use --owner');
    return;
  }
...
function checkCategories(
  ...
  if (config.defaults?.category && !config.categories.includes(config.defaults.category)) {
    push('warn', 'defaults.category', `not declared in categories (${config.defaults.category})`);
  }
}
