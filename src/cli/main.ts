...
cli
  ...
  .option("--format <format>", "config format: json or yaml", {
    default: "json",
  })
...
cli
  ...
  .option(
    "--yes",
    "skip owner selection and use active/default account when possible",
  )
...
cli.command("doctor", "check environment and config").action(async () => {
  await runDoctor();
});
