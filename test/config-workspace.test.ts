...
import { buildCodeWorkspacePlan, planWorkspace } from '../src/workspace/plan-workspace.js';
...
describe('workspace planning', () => {
  it('marks missing repos as fetch and sorts links/folders by category and name', async () => {
    ...
    await mkdir(path.join(config.ghqRoot, 'github.com', 'ts-76', 'life'), { recursive: true });
    ...
    expect(plan.repoLinks.map((repo) => ({ name: repo.name, status: repo.status }))).toEqual([
      { name: 'life', status: 'ready' },
      { name: 'dotfiles', status: 'fetch' },
    ]);
    expect(workspacePlan.folders).toEqual([{ path: 'projects/life' }, { path: 'tools/dotfiles' }]);
  });
});
